...
## 2025-05-15 - [Anti-pattern: N+1 Time-series Fetching]
**Learning:** The `get_timeseries` endpoint fetched history for each metric sequentially, causing N database round-trips and connection overhead.
**Action:** Use window functions like `ROW_NUMBER() OVER(PARTITION BY ...)` to fetch batch "top N" results in a single query.
