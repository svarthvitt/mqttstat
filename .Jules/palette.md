...
## 2026-05-03 - [UX] Transient Feedback for Clipboard Actions
**Learning:** Using `window.alert` for success confirmations like "Copied to clipboard" is disruptive and requires extra user interaction to dismiss. Implementing a transient label change (e.g., "✅ Copied!") on the button itself provides immediate, non-intrusive feedback that respects user flow.
**Action:** Use localized state-driven label transitions with a 2000ms timeout for simple success confirmations.
