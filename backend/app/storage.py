        ...
        with psycopg.connect(self._database_url) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    f"WHERE t.name = %s AND m.ts >= %s AND m.ts <= %s{metric_filter}",
                    tuple(params),
                )
    ...
    def stats(
        ...
        *,
        ...
        with psycopg.connect(self._database_url) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    f"SELECT COUNT(*), MIN(m.value), MAX(m.value), AVG(m.value)::double precision "
                ...
                cur.execute(
                    f"SELECT m.value, m.ts FROM measurements m JOIN topics t ON t.id = m.topic_id "
                    f"WHERE t.name = %s AND m.ts >= %s AND m.ts <= %s{metric_filter} "
                    f"ORDER BY m.ts DESC LIMIT 1",
                    tuple(params),
                )
                latest = cur.fetchone()
        ...
        return TopicStats(
            ...
            minimum=agg[1] if agg else None,
            maximum=agg[2] if agg else None,
            average=agg[3] if agg else None,
            count=int(agg[0] if agg else 0),
            first_value=first[0] if first else None,
            first_observed_at=first[1] if first else None,
            latest_observed_at=latest[1] if latest else None,
        )
