...
class MetricRepository:
        ...
        *,
        ...
        with psycopg.connect(self._database_url) as conn:
            with conn.cursor() as cur:
                ...
                if not rows:
                    # If offset is higher than total count, we get 0 rows.
                    # We need a separate count query to return the true total.
                    cur.execute(
                        """
                        SELECT COUNT(*)
                        FROM measurements m
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    SELECT
                        ...
                        l.value as latest, l.ts as latest_ts,
                        f.value as first, f.ts as first_ts
                    FROM aggregates a
                    LEFT JOIN latest_rec l ON TRUE
                    LEFT JOIN first_rec f ON TRUE
                    """,
                    params,
                )
                row = cur.fetchone()
    ...
    def get_global_stats(
        ...
        *,
        ...
        """
        Optimized method to fetch global KPIs across all topics/metrics in a single SQL query.
