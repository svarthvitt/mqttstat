...
class MetricRepository:
    ...
    def stats(
        ...
        *,
        ...
        where = "WHERE t.name = %s AND m.ts >= %s AND m.ts <= %s"
        params = [topic, start.astimezone(timezone.utc), end.astimezone(timezone.utc)]
        if metric:
            where += " AND m.metric = %s"
        ...
        with psycopg.connect(self._database_url) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    f"FROM measurements m JOIN topics t ON t.id = m.topic_id {where}",
                    tuple(params),
                )
                ...
                cur.execute(
                    ...
                    f"{where} ORDER BY m.ts DESC LIMIT 1",
                    tuple(params),
                )
                ...
                cur.execute(
                    f"SELECT m.value, m.ts FROM measurements m JOIN topics t ON t.id = m.topic_id "
                    f"{where} ORDER BY m.ts ASC LIMIT 1",
                    tuple(params),
                )
