        ...
        *,
        ...
        with psycopg.connect(self._database_url) as conn, conn.cursor() as cur:
            cur.execute(
                ...
                SELECT
                    (SELECT COUNT(*) FROM filtered_measurements) AS count,
                    (SELECT MIN(value) FROM filtered_measurements) AS minimum,
                    (SELECT MAX(value) FROM filtered_measurements) AS maximum,
                    (SELECT AVG(value)::double precision FROM filtered_measurements) AS average,
                    (SELECT value FROM filtered_measurements ORDER BY ts DESC LIMIT 1) AS latest,
                    (SELECT ts FROM filtered_measurements ORDER BY ts DESC LIMIT 1) AS latest_observed_at,
                    (SELECT value FROM filtered_measurements ORDER BY ts ASC LIMIT 1) AS first_value,
                    (SELECT ts FROM filtered_measurements ORDER BY ts ASC LIMIT 1) AS first_observed_at
                """,
                query_params,
            )
            row = cur.fetchone()
        ...
        if not row or row[0] == 0:
            return TopicStats(
                latest=None, minimum=None, maximum=None, average=None, count=0,
                first_value=None, first_observed_at=None, latest_observed_at=None
            )
