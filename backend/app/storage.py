...
class MetricRepository:
    ...
    def stats(self, *, topic: str, start: datetime, end: datetime, metric: str | None) -> TopicStats:
        p = {"topic": topic, "start": start.astimezone(timezone.utc), "end": end.astimezone(timezone.utc), "metric": metric}
        q = """
            WITH f AS (
                SELECT m.value, m.ts FROM measurements m JOIN topics t ON t.id = m.topic_id
                WHERE t.name = %(topic)s AND m.ts >= %(start)s AND m.ts <= %(end)s AND (%(metric)s IS NULL OR m.metric = %(metric)s)
            )
            SELECT
                (SELECT value FROM f ORDER BY ts DESC LIMIT 1), (SELECT MIN(value) FROM f), (SELECT MAX(value) FROM f),
                (SELECT AVG(value)::double precision FROM f), (SELECT COUNT(*) FROM f),
                (SELECT value FROM f ORDER BY ts ASC LIMIT 1), (SELECT ts FROM f ORDER BY ts ASC LIMIT 1),
                (SELECT ts FROM f ORDER BY ts DESC LIMIT 1)
        """
        with psycopg.connect(self._database_url) as conn:
            with conn.cursor() as cur:
                cur.execute(q, p)
                r = cur.fetchone()
        return TopicStats(*r) if r and r[4] > 0 else TopicStats(None, None, None, None, 0, None, None, None)
